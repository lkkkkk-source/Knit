...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate a trained parser on a manifest, with optional random sampling.")
    parser.add_argument("--manifest", type=Path, required=True, help="Source JSONL manifest")
    ...
    parser.add_argument("--sample-size", type=int, default=0, help="If > 0, randomly sample this many rows from the manifest")
    parser.add_argument("--seed", type=int, default=42, help="Random seed used for manifest sampling")
...
def main(argv: list[str] | None = None) -> int:
    ...
    sampled_manifest_path = args.output_dir / "sampled_manifest.jsonl"
    manifest_path = _write_sampled_manifest(args.manifest.resolve(), sampled_manifest_path, args.sample_size, args.seed)
        ...
        for batch in dataloader:
            ...
            for sample_index, prediction_mask in enumerate(predictions):
                ...
                if vis_written < args.num_vis:
                    ...
                    resized_rgb = resize_image(load_rgb_image(sample.image_path), cast(tuple[int, int], image_size), nearest=False)
    ...
    result = {
        ...
        "sampled_manifest": str(manifest_path.resolve()),
