...
def _require_torch() -> object:
    ...
    try:
        ...
        raise ImportError("PyTorch is required for inverse_full_v1 eval. Install with `pip install -e .[train]`.") from error


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate inverse_full_v1.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    refiner.load_state_dict(cast(dict[str, object], checkpoint["refiner_state_dict"]))
    refiner.to(device)
    refiner.eval()
